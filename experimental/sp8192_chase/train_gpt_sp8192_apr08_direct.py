...
def should_use_torch_compile(h,device):
	mode=str(h.use_torch_compile).lower()
	if mode in{'0','false','no'}:return False
	if mode in{'1','true','yes'}:return True
	if device.type!='cuda':return False
	return torch.cuda.get_device_capability(device)[0]==9
def maybe_compile(obj,h,device):
	if should_use_torch_compile(h,device):return torch.compile(obj,dynamic=False,fullgraph=True)
	return obj
...
class CausalSelfAttention(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len):
		...
		kv_dim=self.num_kv_heads*self.head_dim;self.c_q=CastedLinear(dim,dim,bias=False);self.c_k=CastedLinear(dim,kv_dim,bias=False);self.c_v=CastedLinear(dim,kv_dim,bias=False);self.proj=CastedLinear(dim,dim,bias=False);self.proj._zero_init=True;self.q_gain=nn.Parameter(torch.full((num_heads,),qk_gain_init,dtype=torch.float32));self.rope_dims=0;self.rotary=Rotary(self.head_dim,base=rope_base,train_seq_len=train_seq_len);self.use_xsa=False;self.use_flash_attn_3=flash_attn_3_func is not None
	...
	def forward(self,x):
		bsz,seqlen,dim=x.shape;q=self.c_q(x).reshape(bsz,seqlen,self.num_heads,self.head_dim);k=self.c_k(x).reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);v=self.c_v(x).reshape(bsz,seqlen,self.num_kv_heads,self.head_dim);q=F.rms_norm(q,(q.size(-1),));k=F.rms_norm(k,(k.size(-1),));cos,sin=self.rotary(seqlen,x.device,q.dtype);q=apply_rotary_emb(q,cos,sin,self.rope_dims);k=apply_rotary_emb(k,cos,sin,self.rope_dims);q=q*self.q_gain.to(dtype=q.dtype)[None,None,:,None];use_flash=self.use_flash_attn_3 and x.is_cuda and torch.cuda.get_device_capability(x.device)[0]==9
		if use_flash:y=flash_attn_3_func(q,k,v,causal=True)
		else:
			q_sdpa=q.transpose(1,2);k_sdpa=k.transpose(1,2);v_sdpa=v.transpose(1,2)
			if self.num_kv_heads!=self.num_heads:
				kv_repeat=self.num_heads//self.num_kv_heads;k_sdpa=k_sdpa.repeat_interleave(kv_repeat,dim=1);v_sdpa=v_sdpa.repeat_interleave(kv_repeat,dim=1)
...
def eval_val_sliding_ttt(h,base_model,rank,world_size,device,val_data,stride):
	...
	log(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} total_windows={len(window_starts)} stride={stride} ttt_lr={h.ttt_lr} ttt_epochs={h.ttt_epochs} freeze_blocks={h.ttt_freeze_blocks}");compiled_logits=maybe_compile(base_model.forward_logits,h,device);loss_sum=torch.zeros((),device=device,dtype=torch.float64);token_count=torch.zeros((),device=device,dtype=torch.float64);byte_count=torch.zeros((),device=device,dtype=torch.float64);frozen_block_ids=set(range(min(h.ttt_freeze_blocks,len(base_model.blocks))));ttt_params=[]
...
def train_model(h,device,val_data):
	base_model=GPT(h).to(device).bfloat16();restore_fp32_params(base_model);compiled_model=maybe_compile(base_model,h,device)
...
def train_and_eval(h,device):
	...
	compiled_model=maybe_compile(eval_model,h,device);timed_eval('quantized',eval_val,h,device,val_data,compiled_model)
...
def main():
	...
	if h.is_main_process:
		os.makedirs('logs',exist_ok=True);log(f"use_torch_compile: {should_use_torch_compile(h,device)}",console=True);log(100*'=',console=False);log('Hyperparameters:',console=True)
